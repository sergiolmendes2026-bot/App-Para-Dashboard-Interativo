...
if selected == "Dashboard":
    ...
    with st.form("form_venda", clear_on_submit=True):
        ...
        with col_v1:
            ...
            v_produto = st.text_input("Produto / Serviço")
        with col_v2:
        ...
        if btn_venda:
            if v_cliente and v_valor > 0:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (v_cliente, v_valor, v_data, v_responsavel, v_status, v_produto))
                conn.commit()
                conn.close()
                st.success("Venda registrada com sucesso!")
                st.rerun()
            else:
                st.error("Preencha o cliente e o valor da venda.")
    ...
    st.write("Configure os envios automáticos por e-mail:")
    ...
    if st.button("Testar Envio de E-mail Agora"):
        # Cria um relatório em CSV simulado em bytes para teste
        csv_bytes = b"id,cliente,valor\n1,Teste,1000.0"
        sucesso = disparar_email_automatico(destinatario_input, csv_bytes, "relatorio_teste.csv")
        if sucesso:
            st.success("E-mail enviado com sucesso!")
        else:
            st.error("Falha ao enviar o e-mail. Verifique a senha de app.")
