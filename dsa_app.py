...
if selected == "Dashboard":
    ...
    st.info("Utilize o menu lateral para navegar entre as seções, incluindo o novo painel de **Atividades**.")

# --- PÁGINA: ATIVIDADES (COMPLETA E SEGURA) ---
    ...
    # Tratamento de Datas e Contadores (KPIs)
    ...
    hoje_atv = len(df_atv_view[df_atv_view["data"] == hoje_str]) if not df_atv_view.empty else 0
    ...
    kpi3.metric("Hoje", hoje_str if hoje_atv > 0 else hoje_atv) # Exibe contagem clara
    kpi4.metric("Atrasadas", atrasadas_atv)
    kpi5.metric("Concluídas", concluidas_atv)

    st.markdown("<br>", unsafe_allow_html=True)

    # Modal / Formulário de Cadastro de Nova Atividade
    ...
    # Filtragem dos dados exibidos na Agenda
...
# --- OUTRAS PÁGINAS DO SISTEMA (MANTIDAS INTEGRIDADE) ---
